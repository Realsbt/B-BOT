...
// Only limits the command-type token length (e.g. "FORWARD", "INCREASELEGLENGTH").
// Line-level buffers are 256 B; per-sub-command buffers (split by ';') are 128 B.
...
static uint32_t sLastYawRateMs = 0;
static uint32_t sLastDriveMs = 0;
...
static bool canApplyDirectYawRate(void) {
    return cmdQueue.state != EXEC_RUNNING && cmdQueue.state != EXEC_PAUSED;
}
...
void Serial_TryStartQueue(void) {
    if (cmdQueue.count > 0 && cmdQueue.state != EXEC_RUNNING) {
        startQueueExecution();
    }
}
...
int Serial_HandleCommandLine(const char *line) {
    ...
    char rxBuffer[256];
    strncpy(rxBuffer, line, sizeof(rxBuffer) - 1);
    rxBuffer[sizeof(rxBuffer) - 1] = '\0';
    trim_whitespace(rxBuffer);
    ...
    if (strcmp(rxBuffer, "QUEUE_START") == 0) {
        ...
        return 0;
#ifdef ENABLE_WIFI_CMD
    } else if (strcmp(rxBuffer, "VISION_DISABLE") == 0) {
        WiFiCmd_SetInputEnabled(false);
        Serial.println("WiFi vision input disabled via serial");
        return 0;
    } else if (strcmp(rxBuffer, "VISION_ENABLE") == 0) {
        WiFiCmd_SetInputEnabled(true);
        Serial.println("WiFi vision input enabled via serial");
        return 0;
#endif
    } else if (strncmp(rxBuffer, "DRIVE,", 6) == 0) {
        // DRIVE,<speed_milli_mps>,<yaw_mrad_s> — direct teleop, bypasses the queue
        const char *p = rxBuffer + 6;
        char *endp = NULL;
        long speedMilli = strtol(p, &endp, 10);
        long yawMrad = 0;
        if (endp && *endp == ',') {
            yawMrad = strtol(endp + 1, NULL, 10);
        }
        int speedLimitMilli = (int)(MAX_SPEED * 1000.0f);
        int yawLimitMrad = (int)(MAX_YAWSPEED * 1000.0f);
        if (speedMilli > speedLimitMilli) speedMilli = speedLimitMilli;
        if (speedMilli < -speedLimitMilli) speedMilli = -speedLimitMilli;
        if (yawMrad > yawLimitMrad) yawMrad = yawLimitMrad;
        if (yawMrad < -yawLimitMrad) yawMrad = -yawLimitMrad;
        if (canApplyDirectYawRate()) {
            target.speedCmd = speedMilli * 0.001f;
            target.yawSpeedCmd = yawMrad * 0.001f;
            uint32_t now = xTaskGetTickCount() * portTICK_PERIOD_MS;
            sLastDriveMs = (speedMilli == 0 && yawMrad == 0) ? 0 : now;
            sLastYawRateMs = (yawMrad == 0) ? 0 : now;
            Serial.printf("DRIVE set: %ld mm/s, %ld mrad/s\n", speedMilli, yawMrad);
        } else {
            Serial.println("DRIVE ignored: command queue busy");
        }
        return 0;
        ...
        if (canApplyDirectYawRate()) {
            ...
            sLastYawRateMs = (mrad == 0) ? 0 : xTaskGetTickCount() * portTICK_PERIOD_MS;
            Serial.printf("YAWRATE set: %d mrad/s\n", mrad);
        } else {
            Serial.println("YAWRATE ignored: command queue busy");
        }
        return 0;
    }

    int addedCount = 0;
    if (isCompositeCommand(rxBuffer)) {
        Serial.printf("处理复合指令: %s\n", rxBuffer);
        addedCount = parseAndEnqueueSequence(rxBuffer);
...
int Serial_InjectCommandLine(const char *line) {
    return Serial_HandleCommandLine(line);
}
    ...
    while (1) {
        while (Serial2.available() > 0) {
            ...
            if (c == '\n' || bufIndex >= 255) {
                ...
                Serial_HandleCommandLine(rxBuffer);
            }
            else {
                if (bufIndex < 255) {
                    rxBuffer[bufIndex++] = c;
...
void Serial_Task(void *pvParameters) {
    while (1) {
        ...
        if (sLastYawRateMs != 0 &&
            ...
            canApplyDirectYawRate()) {
            target.yawSpeedCmd = 0;
            sLastYawRateMs = 0;
            Serial.println("YAWRATE watchdog: yaw stopped");
        }
        if (sLastDriveMs != 0 &&
            now - sLastDriveMs > DRIVE_WATCHDOG_MS &&
            canApplyDirectYawRate()) {
            target.speedCmd = 0;
            target.yawSpeedCmd = 0;
            sLastDriveMs = 0;
            Serial.println("DRIVE watchdog: direct teleop stopped");
        }
