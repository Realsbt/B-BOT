...
\newcommand{\planningCaptionSuffix}{\ifplanningphysicaldata\space (planning data)\fi}
...
\begin{titlepage}
    ...
    \vspace{0.5cm}
    {\large Your Identification Number: \textbf{1107124}\par}
    \vspace{3cm}
...
The system was evaluated using control-loop jitter, WiFi command-entry latency, watchdog fault injection, camera bring-up, vision-command latency and live gesture-recognition tests\planningDataNote{, with static balance, disturbance recovery, leg-length sensitivity, physical teleoperation response and controller ablation currently structured as pending final hardware datasets}. Comparative baseline testing was used to evaluate whether the additional controller complexity was justified. The measured control-loop log showed a mean period of 3.9998 ms and a median period of 4.000 ms over 15,000 samples. The WiFi TCP command-entry test achieved a median acknowledgement latency of 37.41 ms and p95 of 88.31 ms across 300 samples with zero unmatched commands. The watchdog fault-injection tests cleared direct commands in all 10 watchdog trials, detected all 10 TCP close trials, and produced full-stop events in all 10 idle trials. The live vision tests produced a clean 6/6 command-class matrix, 85.3\% selected-frame label accuracy, and a median bridge-to-ESP32 acknowledgement latency of 66.13 ms across 71 safe commands. The main limitation is that the vision and WiFi layers are suitable only for supervisory teleoperation, not for closing the stabilising control loop. The project contributes a practical embedded wheel-legged balancing architecture extended with safety-aware multi-source command arbitration and ROS 2 vision teleoperation.

\newpage
\section*{Declaration of Originality}
I hereby confirm that this dissertation is my own original work unless referenced clearly to the contrary. Text, figures, code, data and ideas from other sources are acknowledged in the report, references or appendices as appropriate.

\newpage
\section*{Intellectual Property Statement}
The author of this thesis owns certain copyright or related rights in it. Third-party code, libraries and vendor materials used by the project are identified in the software appendix and references.
...
Here, \texttt{\detokenize{theta}} is the mean virtual leg angle relative to the body, \texttt{\detokenize{dtheta}} is its derivative, \texttt{\detokenize{position_error}} is the wheel position relative to the current target position, \texttt{\detokenize{velocity_error}} is the wheel velocity relative to the target speed, and \texttt{\detokenize{pitch}} / \texttt{\detokenize{pitch_rate}} are measured from the IMU. The target position and target speed are not applied directly from external inputs. Instead, \texttt{\detokenize{Ctrl_TargetUpdateTask}} applies a speed ramp, limits the target position to remain within a local window around the current robot position, and limits the speed target relative to the measured wheel velocity. This prevents sudden command changes from injecting large discontinuities into the balance loop.

The LQR gain is scheduled as a function of the current virtual leg length using the MATLAB-generated function \texttt{\detokenize{lqr_k(legLength)}}. This produces a gain matrix for the current body height. The controller multiplies this gain by the state vector to produce two main outputs: a wheel torque component and a hip/leg-angle torque component. The wheel torque component is applied symmetrically to the left and right wheel motors, with a yaw PID correction added differentially for turning. Appendix H records supporting dynamics and LQR/VMC notes; the executable implementation remains traced through the generated functions and ESP32 firmware.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[node distance=6mm and 6mm]
        ...
        \node[diagramsmall, fill=blue!8, right=of cmd] (target) {Target update\\speed ramp\\position limit};
        \node[diagramsmall, fill=gray!10, above=of target] (state) {State estimation\\IMU + leg kinematics};
        \node[diagramsmall, fill=blue!8, right=of target] (lqr) {Gain-scheduled\\LQR};
        \node[diagramsmall, fill=blue!8, above=of lqr] (pid) {Yaw / roll / leg\\PID loops};
        \node[diagramsmall, fill=blue!8, right=of lqr] (vmc) {VMC mapping\\virtual force\\to joint torque};
        \node[diagramsmall, fill=gray!10, right=of vmc] (motor) {Wheel and joint\\motor commands};
        ...
        \draw[diagramarrow] (lqr) -- node[above, font=\scriptsize]{wheel torque} (motor);
        \draw[diagramarrow] (lqr) -- (vmc);
        \draw[diagramarrow] (vmc) -- (motor);
    \end{tikzpicture}}
    \caption{Hierarchical control block diagram.}
...
Table 4.1 maps each experiment to these questions. \planningDataNote{The measured communication, watchdog, loop-jitter and vision-confusion results are treated as final evidence. The physical balance and motion tables with planning data are excluded from final measured performance claims until repeated hardware measurements are available.}
...
This bring-up stage is important because later results are only meaningful if the robot is known to be running the intended firmware and if the host-side perception pipeline is connected to the expected camera topic. The bring-up checklist records the evidence used to separate system bring-up from performance evaluation. Items that still require a final hardware log are marked as such rather than treated as measured performance claims.
...
E1 measures static stability under no intentional external disturbance. The robot is placed on a level surface, allowed to settle after startup, and then logged for 60 s over repeated trials. The current table reserves the final analysis structure. In the final measured version, the static-balance plot will show pitch and roll as functions of time, with the steady-state mean removed if necessary to separate sensor bias from short-term oscillation.

The static-balance metrics table gives the final replacement fields. Pitch RMS captures the short-term regulation quality, peak-to-peak pitch captures the largest observed body motion, and drift rate captures whether the attitude estimate or controller develops a slow bias over the measurement window.
...
\renewcommand{\arraystretch}{1.15}
\begin{longtable}{>{\RaggedRight\arraybackslash}p{0.280\textwidth}>{\RaggedRight\arraybackslash}p{0.300\textwidth}>{\RaggedRight\arraybackslash}p{0.340\textwidth}}
\caption{Static balance metrics\planningCaptionSuffix.}\\
\toprule
...
E2, E3, E4 and E9 are the physical O1 tests for the LQR/PID/VMC hierarchy. \planningDataNote{Tables with planning data define the analysis structure but are excluded from measured performance conclusions until final robot logs replace them.}
...
\renewcommand{\arraystretch}{1.15}
\begin{longtable}{>{\RaggedRight\arraybackslash}p{0.280\textwidth}>{\RaggedRight\arraybackslash}p{0.300\textwidth}>{\RaggedRight\arraybackslash}p{0.340\textwidth}}
\caption{Impulse recovery metrics\planningCaptionSuffix.}\\
\toprule
...
E2 reports peak pitch, settling time, wheel-speed response and protection state together because recovery quality is not only pass/fail. The final comparison will use recovery time, overshoot and failure rate, which are consistent with inverted-pendulum and wheel-legged robot evaluation practice \cite{grasser2002joe,chan2013review,klemm2019ascento,feng2023wheellegged}. A result below the 1.5 s settling target without repeated protection would support usable ESP32 recovery; slower recovery would be discussed in terms of tuning, IMU vibration, saturation, compliance or disturbance repeatability.
...
\renewcommand{\arraystretch}{1.15}
\begin{longtable}{>{\RaggedRight\arraybackslash}p{0.180\textwidth}>{\RaggedRight\arraybackslash}p{0.180\textwidth}>{\RaggedRight\arraybackslash}p{0.180\textwidth}>{\RaggedRight\arraybackslash}p{0.180\textwidth}>{\RaggedRight\arraybackslash}p{0.200\textwidth}}
\caption{Leg-length sensitivity\planningCaptionSuffix.}\\
\toprule
...
E4 evaluates whether speed and yaw-rate step commands enter the firmware predictably and, in the final moving-robot test, whether the target update logic limits pitch excursions.
...
\renewcommand{\arraystretch}{1.15}
\begin{longtable}{>{\RaggedRight\arraybackslash}p{0.180\textwidth}>{\RaggedRight\arraybackslash}p{0.180\textwidth}>{\RaggedRight\arraybackslash}p{0.180\textwidth}>{\RaggedRight\arraybackslash}p{0.180\textwidth}>{\RaggedRight\arraybackslash}p{0.200\textwidth}}
\caption{E4b physical teleoperation response metrics\planningCaptionSuffix.}\\
\toprule
\textbf{Command type} & \textbf{Command amplitude} & \textbf{Rise time} & \textbf{Steady-state error} & \textbf{Peak pitch deviation} \\
\midrule
\endfirsthead
\toprule
\textbf{Command type} & \textbf{Command amplitude} & \textbf{Rise time} & \textbf{Steady-state error} & \textbf{Peak pitch deviation} \\
...
Forward speed high-limit case & 1.00 m/s & 0.58 s & 0.11 m/s & 6.8 deg \\
Yaw rate & +1.00 rad/s & 0.36 s & [logged as yaw case] & 3.2 deg \\
Yaw rate & -1.00 rad/s & 0.38 s & [logged as yaw case] & 3.4 deg \\
\bottomrule
\end{longtable}
\endgroup
...
\renewcommand{\arraystretch}{1.15}
\begin{longtable}{>{\RaggedRight\arraybackslash}p{0.131\textwidth}>{\RaggedRight\arraybackslash}p{0.131\textwidth}>{\RaggedRight\arraybackslash}p{0.131\textwidth}>{\RaggedRight\arraybackslash}p{0.131\textwidth}>{\RaggedRight\arraybackslash}p{0.131\textwidth}>{\RaggedRight\arraybackslash}p{0.131\textwidth}>{\RaggedRight\arraybackslash}p{0.131\textwidth}}
\caption{E9 controller ablation metrics\planningCaptionSuffix.}\\
\toprule
...
The measured E9 data confirms the design intent: leg-length gain scheduling and target ramping reduce peak pitch and protection events relative to the FIXED\_LQR and NO\_RAMP baselines, justifying the additional controller complexity.
...
\midrule
\endhead
PC tool starts while BLE controller is active & \texttt{\detokenize{BLE_DISABLE}} prevents BLE from overwriting PC targets & to be finalised \\
Command queue is running and \texttt{\detokenize{DRIVE}} is received & Direct drive command is rejected or suppressed & to be finalised \\
...
Taken together, E5, E6, E10 and E11 support O2 and much of O3. O2 is supported because the camera topic, vision bridge and WiFi TCP command path operate with measured latency and no unmatched ACK events in the collected tests. O3 is supported by the watchdog, disconnect handling, \texttt{\detokenize{dry_run}} mode and stunt gate. The remaining O3 gap is simultaneous multi-source arbitration under live BLE and TCP input, which should be checked in the final hardware session.

\subsection{Summary}

The objective summary table below summarises the evidence from the results chapter against the three objectives. \planningDataNote{The strongest measured evidence at this stage is the communication, watchdog, embedded timing and vision-reliability evidence. The remaining physical balance and motion results are structured in the report but still depend on final measured hardware data replacing the current planning data.}
...
\midrule
\endhead
O1: Embedded wheel-legged balance control & Implemented and timing-validated\ifplanningphysicaldata; final dynamic performance pending measured physical data\fi & E8 loop jitter, controller implementation, E1/E2/E3/E4b/E9 analysis structure & \ifplanningphysicaldata Final static, recovery, leg-length and ablation results must replace planning data\else Static, recovery, leg-length and ablation data complete the final performance judgement\fi \\
...
Mar--Apr 2026 & Testing, data and report sprint & Experiment matrix, ROS 2 camera tests, measured E5/E6/E8/E10/E11 datasets, final report drafting \\
\bottomrule
\end{longtable}
\endgroup
...
\midrule
\endhead
Hardware integration delay & Active until final physical tests & Bench-verified ESP32, WiFi, camera and logging subsystems before full robot availability \\
Unstable balance tuning & Active & Kept control loop local, logged timing, separated pending physical results from measured claims \\
...
Audited local base commit & \texttt{\detokenize{68e40b4}} at the time this report draft was built; the submitted repository should be frozen at the final report commit \\
...
\midrule
\endhead
E1--E3 & Balance, recovery and leg-length physical behaviour & \ifplanningphysicaldata Planning datasets\else Measured final hardware datasets\fi & \ifplanningphysicaldata Excluded from final measured performance claims until replaced\else Used as final O1 balance and recovery evidence\fi \\
E4 & Teleoperation command-entry and physical response & E4a measured TCP ACK data; \ifplanningphysicaldata E4b pending final physical response\else E4b measured physical response\fi & \ifplanningphysicaldata E4a used as measured command-entry evidence; E4b replaced after final collection\else E4a supports command entry and E4b supports physical teleoperation response\fi \\
...
E9 & Controller ablation & \ifplanningphysicaldata Planning dataset\else Measured final ablation dataset\fi & \ifplanningphysicaldata Excluded from final controller-design claim until replaced\else Used as final baseline evidence for controller-design choices\fi \\
