...
% ==========================================
\usepackage{graphicx}
\graphicspath{{figures/}{figures/provisional/}}
\usepackage{float}
\usepackage{placeins}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[node distance=5mm and 8mm]
        \node[diagramblock, fill=gray!10] (body) {Central body\\ESP32\\battery\\IMU};
...
\begin{figure}[H]
    ...
    \resizebox{\textwidth}{!}{%
    \begin{tikzpicture}[node distance=6mm and 7mm]
        \node[diagramblock, fill=blue!8] (esp) {ESP32 controller\\FreeRTOS firmware};
        ...
        \node[diagramblock, fill=gray!10, right=of esp] (can) {CAN bus\\motor feedback\\motor commands};
        \node[diagramsmall, fill=gray!10, right=of can] (motors) {six motor drivers\\four joints\\two wheels};
...
Table 4.1 maps each experiment to these questions. The measured communication, watchdog, loop-jitter and vision-confusion results are treated as final evidence. The physical balance and motion rows marked \textit{[PROVISIONAL]} are synthetic planning placeholders and are excluded from final measured performance claims until repeated hardware measurements are available.
...
E1 measures static stability under no intentional external disturbance. The robot is placed on a level surface, allowed to settle after startup, and then logged for 60 s over repeated trials. The current dataset is synthetic and is used only to reserve the analysis structure. In the final measured version, the static-balance plot will show pitch and roll as functions of time, with the steady-state mean removed if necessary to separate sensor bias from short-term oscillation.

\noindent\textit{Provisional placeholder figure for Static balance drift over a 60 s window is stored in Appendix G as \path{Report/figures/provisional/e1_static_balance_drift_provisional.png}. It is excluded from the final measured performance claim until replaced by hardware data.}
...
\midrule
\endhead
Pitch RMS & 0.291 deg \textit{[PROVISIONAL]} & Planned pass criterion is below 0.5 deg \\
Roll RMS & 0.231 deg \textit{[PROVISIONAL]} & Indicates lateral body stability \\
Pitch peak-to-peak & 1.37 deg \textit{[PROVISIONAL]} & Captures worst short-term body motion \\
Roll peak-to-peak & 1.04 deg \textit{[PROVISIONAL]} & Captures lateral oscillation \\
Pitch drift rate & 0.0138 deg/s \textit{[PROVISIONAL]} & Expected to remain close to zero after IMU calibration \\
Failed/protected trials & 0/3 \textit{[PROVISIONAL]} & Any protection event must be discussed \\
\bottomrule
\end{longtable}
\endgroup
...
E2, E3, E4 and E9 are the physical O1 tests for the LQR/PID/VMC hierarchy. Values marked \textit{[PROVISIONAL]} are synthetic placeholders: they define the analysis structure but are excluded from measured performance conclusions until final robot logs replace them.
...
\noindent\textit{The provisional E2 recovery plot is stored in Appendix G as \path{Report/figures/provisional/e2_recovery_curves_synthetic_provisional.png}.}
...
\midrule
\endhead
Trials & 10 \textit{[PROVISIONAL]} & 10 \textit{[PROVISIONAL]} \\
Successful recoveries & 9/10 \textit{[PROVISIONAL]} & 10/10 \textit{[PROVISIONAL]} \\
Peak pitch deviation & 9.05 deg mean \textit{[PROVISIONAL]} & 7.46 deg mean \textit{[PROVISIONAL]} \\
Settling time & 0.867 s mean over successful trials \textit{[PROVISIONAL]} & 0.783 s mean \textit{[PROVISIONAL]} \\
Maximum wheel speed & 19.17 rad/s mean \textit{[PROVISIONAL]} & 17.65 rad/s mean \textit{[PROVISIONAL]} \\
Protection triggers & 1 \textit{[PROVISIONAL]} & 0 \textit{[PROVISIONAL]} \\
\bottomrule
\end{longtable}
\endgroup
...
\noindent\textit{The provisional E3 leg-length plot is stored in Appendix G as \path{Report/figures/provisional/e3_leg_length_synthetic_provisional.png}.}
...
\midrule
\endhead
Minimum & 0.055 m \textit{[PROVISIONAL]} & 0.641 s \textit{[PROVISIONAL]} & 6.22 deg \textit{[PROVISIONAL]} & 0/5 \textit{[PROVISIONAL]} \\
Middle & 0.070 m \textit{[PROVISIONAL]} & 0.828 s \textit{[PROVISIONAL]} & 7.99 deg \textit{[PROVISIONAL]} & 0/5 \textit{[PROVISIONAL]} \\
Maximum & 0.085 m \textit{[PROVISIONAL]} & 1.150 s \textit{[PROVISIONAL]} & 11.22 deg \textit{[PROVISIONAL]} & 1/5 \textit{[PROVISIONAL]} \\
\bottomrule
\end{longtable}
\endgroup

The provisional trend increases settling time and peak pitch at larger leg length. If measured, this would define a safe operating envelope and motivate gain retuning rather than being treated only as a failure.
...
\noindent\textit{The provisional E4b physical-response plot is stored in Appendix G as \path{Report/figures/provisional/e4b_physical_step_synthetic_provisional.png}.}
...
\midrule
\endhead
Forward speed & 0.30 m/s \textit{[PROVISIONAL]} & 0.31 s \textit{[PROVISIONAL]} & 0.03 m/s \textit{[PROVISIONAL]} & 2.4 deg \textit{[PROVISIONAL]} \\
Forward speed & 0.60 m/s \textit{[PROVISIONAL]} & 0.42 s \textit{[PROVISIONAL]} & 0.06 m/s \textit{[PROVISIONAL]} & 4.1 deg \textit{[PROVISIONAL]} \\
Forward speed high-limit case & 1.00 m/s \textit{[PROVISIONAL]} & 0.58 s \textit{[PROVISIONAL]} & 0.11 m/s \textit{[PROVISIONAL]} & 6.8 deg \textit{[PROVISIONAL]} \\
Yaw rate & +1.00 rad/s \textit{[PROVISIONAL]} & 0.36 s \textit{[PROVISIONAL]} & [not measured separately] \textit{[PROVISIONAL]} & 3.2 deg \textit{[PROVISIONAL]} \\
Yaw rate & -1.00 rad/s \textit{[PROVISIONAL]} & 0.38 s \textit{[PROVISIONAL]} & [not measured separately] \textit{[PROVISIONAL]} & 3.4 deg \textit{[PROVISIONAL]} \\
\bottomrule
\end{longtable}
\endgroup
...
E9 compares the full controller with \texttt{\detokenize{FIXED_LQR}} and \texttt{\detokenize{NO_RAMP}} variants. It tests whether leg-length gain scheduling and target ramping are justified design choices rather than untested complexity.

\noindent\textit{The provisional E9 ablation plot is stored in Appendix G as \path{Report/figures/provisional/e9_ablation_synthetic_provisional.png}.}
...
\midrule
\endhead
FULL & impulse recovery & 10 \textit{[PROVISIONAL]} & 10/10 \textit{[PROVISIONAL]} & 0.826 s \textit{[PROVISIONAL]} & 8.09 deg \textit{[PROVISIONAL]} & 0 \textit{[PROVISIONAL]} \\
FIXED\_LQR & impulse recovery & 10 \textit{[PROVISIONAL]} & 9/10 \textit{[PROVISIONAL]} & 1.049 s \textit{[PROVISIONAL]} & 9.85 deg \textit{[PROVISIONAL]} & 1 \textit{[PROVISIONAL]} \\
NO\_RAMP & drive step & 10 \textit{[PROVISIONAL]} & 10/10 \textit{[PROVISIONAL]} & 0.476 s rise \textit{[PROVISIONAL]} & 9.17 deg \textit{[PROVISIONAL]} & 0 \textit{[PROVISIONAL]} \\
\bottomrule
\end{longtable}
\endgroup

If the final E9 data confirms the provisional trend, it will justify gain scheduling and ramping through lower pitch excursion or fewer protection events. If not, the added complexity should be removed or retuned.
...
Unstable balance tuning & Active & Kept control loop local, logged timing, separated provisional physical results from measured claims \\
WiFi / vision unreliability & Managed & Used watchdog expiry, TCP full-stop injection, dry-run default and stunt arming \\
Insufficient measured data & Active & Created E1--E11 matrix and explicit measured/provisional data index \\
...
Third-party software boundary & Yahboom base code, ESP32/Arduino, FreeRTOS, ROS 2, micro-ROS, OpenCV, MediaPipe, NimBLE and sensor libraries are listed in the attribution appendix \\
\bottomrule
\end{longtable}
\endgroup

\section{Experiment Data, Logs and Extra Validation Evidence}
The experiment data index is stored in \path{Report/appendices/E_data/README.md}. Raw CSV files, summary files, smoke-test evidence and supporting scripts are stored in \path{Report/appendices/E_data/}. Measured and provisional datasets are explicitly separated. Provisional or synthetic rows are excluded from measured performance claims until they are replaced by hardware data.
...
\midrule
\endhead
E1--E3 & Balance, recovery and leg-length physical behaviour & Provisional/synthetic planning datasets & Excluded from final measured performance claims until replaced \\
E4 & Teleoperation command-entry and physical response & E4a measured TCP ACK data; E4b provisional physical response & E4a used as measured command-entry evidence; E4b marked provisional \\
...
E9 & Controller ablation & Provisional/synthetic planning dataset & Excluded from final controller-design claim until replaced \\
