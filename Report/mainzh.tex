...
表 4.1 将每个实验映射到这些问题。E1--E11 数据集均已实测，并在正文中以汇总指标作为最终证据；原始时间序列和逐试验日志保留在附录 G。
...
实测 E9 数据在比较测试层面支持设计意图。FULL 与 FIXED\_LQR 两行是直接的冲击恢复比较：完整控制器 10 次全部恢复，并相对固定增益版本降低了响应时间和峰值 pitch。NO\_RAMP 行是 drive 阶跃消融，而不是同一个冲击测试，因此只作更窄解释：去掉目标斜坡会在突变 drive 命令下增加 pitch 偏移，支持斜坡设计，但不把它夸大成一一对应的恢复比较。
...
输入仲裁行为以简短 pass/fail 表报告，因为它是系统的主要工程贡献。E6 和 E10/E11 覆盖了已实测的故障与视觉安全案例。两个同时输入案例保留为剩余验证项，因为它们没有被 E1--E11 日志单独覆盖。
...
\midrule
\endhead
BLE 控制器活动时 PC 工具启动 & \texttt{\detokenize{BLE_DISABLE}} 防止 BLE 覆盖 PC 目标 & 剩余验证项；未在 E1--E11 中单独测量 \\
命令队列运行时收到 \texttt{\detokenize{DRIVE}} & 直接 drive 命令被拒绝或抑制 & 剩余验证项；未在 E1--E11 中单独测量 \\
...
综合来看，E5、E6、E10 和 E11 支持 O2 和 O3 的大部分内容。O2 得到支持，因为摄像头话题、视觉桥接和 WiFi TCP 命令路径在已收集测试中以实测延迟运行，且没有未匹配 ACK 事件。O3 由看门狗、断开处理、\texttt{\detokenize{dry_run}} 模式和 stunt gate 支持。剩余 O3 缺口是实时 BLE 和 TCP 输入下的多源同时仲裁，应作为未来回归测试，而不是已完成结果。
...
\textbf{提交版本：} public \texttt{\detokenize{main}} 分支，commit \texttt{\detokenize{0e2114c}}；完整快照链接：\url{https://github.com/Realsbt/B-BOT/tree/0e2114cfccb35874be668d403cf52bcf92e890e1}
...
提交仓库状态 & public \texttt{\detokenize{main}} 分支，commit \texttt{\detokenize{0e2114c}}；完整快照：\url{https://github.com/Realsbt/B-BOT/tree/0e2114cfccb35874be668d403cf52bcf92e890e1} \\
