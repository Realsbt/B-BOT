# 自平衡轮腿机器人 micro-ROS 与 WiFi Final Report 可执行写作大纲与自查报告

## 执行摘要

对“自平衡轮腿机器人 + micro-ROS + WiFi”这类项目，final report 的最佳写法不是按开发时间线记录“我先做了什么、后来修了什么”，而是按“工程目标—系统分层—关键权衡—验证证据”来展开。原因在于，micro-ROS 本身就是一个把微控制器节点通过 agent 接入标准 ROS 2 系统的分层架构；其底层还牵涉 XRCE-DDS、transport、QoS 兼容性与有损无线链路的行为。如果这些关系不在文稿里显式说明，读者就很难判断你的控制与通信设计是否合理、哪些结果能归因于控制器、哪些结果又受 WiFi 与消息配置影响。citeturn4view0turn4view1turn5view4turn5view5turn6search2
...
在通用报告写作上，高校官方写作指南普遍强调几件事：abstract 应是独立、自足的全文快照；literature review 不能只是摘抄或逐篇摘要，而应按主题组织并作批判性分析；discussion 需要解释结果含义、与已有研究比较、指出限制和后续方向；通用模板与建议都要服从课程文件和具体读者需求。下文的大纲，就是在这些稳定原则之上，再结合 micro-ROS、ROS 2、提交平台、代码文档与风险评估的官方资料，构造出的执行版模板。citeturn14search0turn14search11turn13search6turn13search1turn15search8turn15search10

| 学校专属项目 | 当前状态 | 执行动作 |
|---|---:|---|
| Title page 精确字段与版式 | 未指定 | 以学校模板第一页为唯一真源，**原样套用** |
| Declaration of originality 固定文字 | 未指定 | 若模板有固定页，整页原样保留 |
| Intellectual property statement 固定文字 | 未指定 | 若模板有固定页，整页原样保留 |
| Abstract 具体字数/页数 | 未指定 | 先按 1 页模板写；再按学校要求压缩或扩展 |
| 正文 35 页的统计口径 | 你已给定“正文 35 页” | 最保守做法：正文控制在 32–33 页，预留 2–3 页缓冲 |
| 提交平台是否为 Blackboard | 未指定 | 以课程 handout / VLE 页面为准 |
| 迟交扣分或零分规则 | 未指定 | 必须回查 handbook / assessment brief |
| 评分 rubric / mark breakdown | 未指定 | 必须找到 assessment brief 或 marking criteria |
| 是否匿名评分 | 未指定 | 若是，则从文件名、封面与正文中移除身份信息 |

对这个题目而言，真正有“技术特异性”的写作依据主要来自四类官方资料：其一，micro-ROS/ROS 2 官方文档解释了 MCU、agent、QoS 与有损网络的设计边界；其二，README 与软件引用文档解释了如何把代码与复现信息打包为可审查证据；其三，风险评估官方模板说明了风险文件应该记录什么；其四，提交平台文档说明了回执、late 标记、rubric 与匿名评分的实际工作方式。citeturn4view0turn10search0turn11search0turn21search2turn23search0turn24search9
...
|---|---:|---|---|---|
| Introduction：背景与工程动机 | 0.8–1.2 页 / 300–500 词 | 为什么是轮腿自平衡？为什么要引入 micro-ROS 与 WiFi？问题的工程价值是什么？ | 图：**项目目标与系统边界概览** | 从“应用场景 + 约束”切入，不从“我一开始做了什么”开头 |
...
| Literature review：嵌入式与 micro-ROS 架构 | 1.0–1.2 页 / 350–500 词 | MCU—agent—host 分层的意义是什么？ | 图：**嵌入式 ROS 2 分层架构示意图** | 讲“为什么这种分层能服务你的项目”，不是泛泛介绍 ROS |
| Literature review：WiFi 与 QoS | 0.8–1.0 页 / 250–400 词 | 有损链路对 sensor/command/telemetry 有什么不同影响？ | 表：**QoS policy 与适用消息类型对照表** | 把通信写成**设计问题**，不是“连上就算做完” |
...
| Methods：micro-ROS 通信设计 | 1.2–1.8 页 / 450–700 词 | 节点、topic、service、QoS、频率、agent 部署、WiFi 拓扑如何设计？ | 表：**Topic / Type / Rate / QoS Matrix**；图：**ROS Graph / Agent Topology Screenshot** | 把“为什么这样配 QoS”和“哪些流量不能走慢链路”讲清楚 |
...
| Results：micro-ROS + WiFi 通信性能 | 1.8–2.2 页 / 650–900 词 | 延迟、掉包、topic 兼容性、CPU/RAM 或消息吞吐如何？ | 图：**End-to-End Latency CDF**；图：**Packet Loss by Topic/QoS**；表：**Communication Performance Matrix** | 把通信结果与控制影响联系起来，而不是做成网络实验孤岛 |
...
而在 methods 部分，**system architecture 与 execution model** 值得写深。micro-ROS 官方文档显示，micro-ROS 采用 rcl+rclc 与 XRCE-DDS，把 MCU 节点经 agent 无缝接入标准 ROS 2；其 transport 支持 serial、UDP 以及 Wi‑Fi 等；rclc executor 还能提供 trigger conditions、顺序执行与 LET 语义，天然适合把“板端快环控制”和“上位机慢环监控/记录/可视化/参数管理”分开写。对于 WiFi，ROS 2 文档同时提醒：在有损网络中，best effort 往往比 reliable 更适用于 sensor data，且 QoS 不兼容会直接阻断 publisher/subscriber 连接。你在 report 中把这些决策写清楚，等于提前回答了评阅人最可能质疑的问题。citeturn4view0turn4view1turn10search0turn5view2turn5view4turn5view5turn5view6
...
说明工程问题、约束条件、为什么采用轮腿 + micro-ROS + WiFi 这一路线。
...
## Topics / Services / QoS
| Name | Type | Direction | Rate | QoS | Purpose |
...
下面给出一个**从 2026-04-23 起算的 28 天倒排模板**。如果你的 deadline 更近，把第 1 周和第 2 周压缩合并；但**不要压缩最后 4 天**，因为最终排版、交叉核对、导出、再上传与回执保存，是最容易出低级错误的阶段。

```mermaid
gantt
    title Final Report 倒排 28 天模板
...
你可以把这个计划理解成五个里程碑：**规则冻结、结构冻结、图表冻结、论证闭环、提交闭环**。其中“图表冻结”尤其重要：你的 discussion 应该围绕图表和表格来写，而不是图表最后再补。只要图表没定，results 与 discussion 就会不断返工。

**Methods 中 system architecture 的示例段落**

本文系统采用三层分层架构。底层由 MCU 负责 \[f_fast] Hz 的姿态估计、电机闭环与安全联锁，以保证所有时延敏感的平衡控制都在板端完成；中间层由 micro-ROS client 与 agent 组成，负责在 MCU 与 ROS 2 主机之间转发遥测数据、参数更新与测试命令；上层主机则承担日志记录、可视化、实验编排与非实时分析任务。这样的拆分将“必须确定性执行”的控制回路与“可以容忍抖动”的监控回路分离，从而降低 WiFi 抖动和消息队列积压对稳定性的直接影响。对于高频传感消息，本文采用 \[QoS 方案]；对于必须可靠送达的配置与控制命令，则采用 \[QoS 方案]，以匹配不同信息流的实时性与可靠性要求。citeturn4view0turn4view1turn5view4turn5view6turn10search0

**Results 中 discussion 的示例段落**

在 \[工况 A] 下，系统能够在遭受 \[扰动类型] 后恢复直立，但恢复时间和峰值超调均随 \[腿长/速度/负载] 增大而上升。单看曲线，这一结果似乎只说明“性能变差”；但更重要的解释是，它揭示了系统瓶颈已经从执行器输出能力转移到状态估计质量与环路时延管理上。结合 \[日志/图号] 可以看到，扰动后最初阶段的控制输入并未立即饱和，而姿态估计噪声与命令抖动却显著上升，这表明在当前架构下，进一步提升稳定性未必首先依赖更强的电机，而更可能依赖更稳健的估计器、更加严格的 QoS 配置，以及对无线链路扰动的隔离策略。与 \[文献基线] 相比，本文系统在 \[某指标] 上表现 \[更好/更弱]，其主要原因在于 \[结构变化、控制简化、实验工况更严格或硬件资源限制]。

**Conclusion 的示例段落**

本文围绕“轮腿自平衡 + micro-ROS + WiFi”的系统目标，完成了从机械/电气集成、板端控制到 ROS 2 通信与验证流程的整体实现。结果表明，所提出的分层架构能够在 \[核心工况] 下实现 \[核心能力]，并且通过 \[实验类型] 证明了系统在 \[指标] 方面达到预期。然而，当前系统仍然受到 \[局限 1]、\[局限 2] 与 \[局限 3] 的约束，因此其主要贡献并不在于“全部性能最优”，而在于给出了一套在受限硬件与无线链路条件下可落地、可调试、可复现的工程实现路径。后续工作应优先面向 \[最高优先级改进]，其次才是 \[次级增强功能]。

**一页 Abstract 模板示例**

本文面向 \[应用场景] 中轮腿机器人在 \[复杂地面 / 姿态变化 / 无线部署] 条件下面临的平衡控制与系统集成问题，设计并实现了一套基于 micro-ROS 与 WiFi 的自平衡轮腿机器人系统。针对传统两轮自平衡系统在 \[关键局限] 方面的不足，本文提出了由板端实时控制、micro-ROS 通信桥接与上位机监控分析组成的分层架构，其中 MCU 负责 \[高频任务]，ROS 2 主机负责 \[非实时任务]，二者通过 \[transport / agent 部署方式] 完成数据交互。为验证系统有效性，本文构建了 \[静态平衡、扰动恢复、移动/变高、通信性能] 等实验，并以 \[恢复时间、超调、掉包率、端到端延迟、CPU/RAM 占用] 作为主要评价指标。实验结果表明，该系统能够在 \[主要工况] 下实现 \[核心结果]，其中在 \[某类测试] 中表现出 \[主要发现]；同时，结果也揭示了 \[主要限制]，说明当前性能瓶颈更多来自 \[估计 / 通信 / 结构 / 参数整定] 而非 \[另一因素]。总体而言，本文的贡献在于：其一，完成了 \[关键系统集成贡献]；其二，建立了 \[验证流程 / 证据链 / 可复现实验框架]；其三，为后续开展 \[更强控制 / 更稳通信 / 更复杂运动能力] 提供了可扩展的工程基础。尽管当前系统仍存在 \[限制项]，但其设计与实验结果已经证明该架构对 \[目标应用或研究方向] 具有可行性与进一步优化价值。
...
| 很高 | micro-ROS 官方文档 | 架构、agent、XRCE-DDS、supported hardware、executor、transport、QoS 相关实现 |
| 很高 | ROS 2 官方文档 | QoS、lossy network 行为、topic/service 语义、与你所用发行版一致的 API |
...
中文资源的优先顺序建议写成：**学校官方中文文件 > 学校图书馆已授权的中文数据库与学位论文库 > 中文综述或硕博论文 > 英文原始论文与官方文档**。如果你有校内授权，中文数据库可用来快速建立主题地图；例如，万方提供机构登录与学术检索服务，适合先搜“轮腿机器人”“自平衡机器人”“LQR ADRC”“ROS 2 嵌入式”等中文关键词。但一旦涉及 micro-ROS/ROS 2 的 API、QoS、executor、transport 与发行版差异，仍应以官方文档为准，因为 ROS 2 文档同时明确区分了最新发布版与开发版，若你混用了不同发行版的资料，很容易把命令、接口和行为写错。citeturn25search0turn4view0turn10search0turn6search4turn6search5